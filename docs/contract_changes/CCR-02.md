...
Why (the failing case or missing capability):
  ...
  @k (22 Sep, decisions/B0.md #26): DET-001 is the first stretch rule, only via a CCR adding its goldens.
  B1 built it (`noteguard/engine/rules/deterioration.py`, B1.2) with the rule DISABLED, so no golden moved.

Semantics as built (each is a question for @k below):
...
Questions for @k (defaults are what B1 built):
...
Lanes affected and what each must do:
  ...
  @k: sign off the new/changed scenarios (SIGNOFF), answer questions 1-5.
  B1: flip `enabled` in the same merge; adjust the rule if @k picks an alternative.
...
Status: approved with defaults (@k, 22 Sep 2026, chat B1.2). Landing prepared by B1 on branch `ccr-01-02`
